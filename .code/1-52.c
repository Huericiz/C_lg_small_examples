...
void odd(unsigned startdigit,unsigned char startrow,unsigned char startcol,unsigned char rowcol)
{
   ...
   for(i=startdigit+1;i<=startdigit+rowcol*rowcol-1;i++)
   {
     ...
     {
        row=row0;
        if(row+1>startrow+row-1) row=startrow;
        else row++;
        col=col0;
        matrix[row][col]=i;
     }
...
void odd_even(unsigned char rowcol)
{
   ...
   for(col=0;col<=k/2-1;col++)
    ...
    {
        t=matrix[row][rowcol-col-1];
        matrix[row][rowcol-col-1]=matrix[row-k][row-col-1];
        matrix[row-k][rowcol-col-1]=t;
    }
}
void even_even(unsigned char rowcol)
{
...
void print(unsigned char rowcol)
{
   ...
   for(i=0;i<rowcol;i++)
   {
     ...
     for(j=0;j<rowcol;j++) printf("\xb3%4d",matrix[i][j]);
     printf("\xb3\n");
   }
   printf("\xc0%s",str);
   for(j=1;j<rowcol;j++) printf("\xcl%s",str);
   printf("\xd9\n");
}
void verify(unsigned char rowcol)
{
