...
double ellipse2(double k,double f)
{
  int m,i,j;
  ...
  do
  {
     ...
     p=g;m<<=1;
  }while((ep>=0.000001)&&(fabs(h)>s));
  return g;
}
double ellipseintegral2(double k,double f)
{
  int n;
  double y,e,ff;
  ...
  {
    ff=ellipse2(k,PI/2.0);
    ff=2.0*n*ff+e*ellipse2(k,y);
  }
  if(f<0.0) ff=-ff;
  return ff;
}
main()
{
  ...
  for(n=1;n<6;n++)
  {
    printf("\nellipseintegral2(0.5,%3.1f)=%5.2f",
           PI*n/6,ellipseintegral2(0.5,PI*n/6));
  }
  getch();
}
